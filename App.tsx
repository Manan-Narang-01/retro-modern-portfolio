...
import { Github, Linkedin, Twitter, Mail, ExternalLink, Download, ChevronRight, GraduationCap, Check } from 'lucide-react';
  ...
  const renderHome = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <RetroCard title="ABOUT_ME" className="h-full flex flex-col">
          ...
          <div className="mt-6 flex gap-4 border-t-2 border-gray-100 pt-4">
            {data.socialLinks.map((link) => {
               const Icon = link.iconName === 'Github' ? Github : link.iconName === 'Linkedin' ? Linkedin : link.iconName === 'Mail' ? Mail : Twitter;
               return (
                 <a key={link.platform} href={link.url} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-black transition-colors transform hover:scale-110">
                   <Icon size={24} />
                 </a>
               )
            })}
        ...
        <RetroCard title="SKILL_MATRIX" className="h-full">
          ...
          <div key={exp.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 border-2 border-black bg-white hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 gap-2">
                ...
                <span className="text-xs bg-black text-white px-2 py-1 font-mono">{exp.duration}</span>
              </div>
        ...
        {data.projects.map((proj) => (
          <RetroCard key={proj.id} title={proj.featured ? "FEATURED" : "STANDARD"} className="h-full flex flex-col">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-black group-hover:underline decoration-2 transition-colors">{proj.title}</h3>
              <a href={proj.link} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-black transition-colors">
                <ExternalLink size={20} />
              </a>
            </div>
            <p className="text-sm text-gray-600 mb-6 flex-grow font-medium">{proj.description}</p>
            <div className="mt-auto space-y-4">
              <div className="flex flex-wrap gap-2">
                {proj.technologies.map(tech => (
                  <span key={tech} className="text-[10px] uppercase font-bold px-2 py-1 bg-gray-100 border border-gray-300 text-gray-800">{tech}</span>
                ))}
              </div>
              <a href={proj.link} target="_blank" rel="noreferrer" className="block">
                <RetroButton variant="secondary" className="w-full text-sm py-2">View Source</RetroButton>
              </a>
            </div>
          </RetroCard>
        ))}
            ...
            {data.education.map((edu) => (
              <RetroCard key={edu.id}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {edu.coursework.map(c => <span key={c} className="text-xs text-black bg-gray-100 px-1">{c}</span>)}
                  </div>
                </div>
              </RetroCard>
            ))}
        ...
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[60] w-[90%] max-w-md animate-in zoom-in slide-in-from-top-4 duration-300">
          <RetroCard title="SYSTEM_MESSAGE" className="bg-white border-black border-4 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center gap-4">
              <Check className="text-green-600 w-10 h-10 shrink-0" />
              <div>
                <h4 className="font-bold text-lg leading-tight">TRANSMISSION SUCCESSFUL</h4>
                <p className="text-sm text-gray-600 mt-1">Data packet received. Redirecting home...</p>
              </div>
            </div>
          </RetroCard>
        </div>
      ...
      <RetroCard className={`p-8 transition-opacity duration-300 ${showNotification ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
        ...
        <form className="space-y-6" onSubmit={handleFormSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" id="name" name="name" className="w-full bg-gray-50 border-2 border-gray-300 p-3 text-black focus:outline-none focus:border-black focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all placeholder-gray-400" placeholder="Name" required />
            </div>
            <div className="space-y-2">
              <label htmlFor="email" className="text-xs font-bold uppercase text-gray-500">Return_Path</label>
              <input type="email" id="email" name="email" className="w-full bg-gray-50 border-2 border-gray-300 p-3 text-black focus:outline-none focus:border-black focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all placeholder-gray-400" placeholder="Email" required />
            </div>
          </div>
          <div className="space-y-2">
            <label htmlFor="message" className="text-xs font-bold uppercase text-gray-500">Data_Packet</label>
            <textarea id="message" name="message" rows={5} className="w-full bg-gray-50 border-2 border-gray-300 p-3 text-black focus:outline-none focus:border-black focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all placeholder-gray-400" placeholder="Type your message..." required></textarea>
          </div>
        ...
        <div className="mt-12 flex justify-center gap-8 border-t-2 border-gray-100 pt-8">
          {data.socialLinks.map((link) => {
             const Icon = link.iconName === 'Github' ? Github : link.iconName === 'Linkedin' ? Linkedin : link.iconName === 'Mail' ? Mail : Twitter;
             return (
               <a key={link.platform} href={link.url} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-2 text-gray-500 hover:text-black group">
                 <div className="p-3 border-2 border-gray-200 rounded-full group-hover:border-black group-hover:bg-gray-50 transition-all"><Icon size={20} /></div>
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-transparent text-black selection:bg-black selection:text-white font-mono transition-opacity duration-1000 ${hasBooted ? 'opacity-100' : 'opacity-0'}`}>
        <RetroBackground />
        <Navbar currentSection={currentSection} onNavigate={setCurrentSection} mobileMenuOpen={mobileMenuOpen} setMobileMenuOpen={setMobileMenuOpen} />
